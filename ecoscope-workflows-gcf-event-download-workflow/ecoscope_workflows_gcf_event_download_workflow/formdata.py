...
class GetEventData(BaseModel):
    ...
    event_types: List[str] = Field(
        ...,
        description='Specify the event type(s) to download (optional). Enter the VALUE (not display name) for each event type (e.g. "giraffe_survey_encounter_nam"). Values can be found in EarthRanger Admin under Activity > Event Types. Leave empty to include all event types.',
        title="Event Types",
    )
...
class ProcessColumns(BaseModel):
    ...
    drop_columns: Optional[List[str]] = Field(
        ...
        description="Columns to remove from the export. Defaults match the GCF Twiga Tools Event Download page. Adjust as needed.",
        title="Drop Columns",
    )
...
class PersistEvents(BaseModel):
    ...
    filetypes: Optional[List[Filetype]] = Field(
        ["csv"],
        description="Output file format. CSV opens in Excel. Parquet preserves geometry and is recommended for spatial analysis.",
        title="Filetypes",
    )
...
class FormData(BaseModel):
    ...
    Process_Events: Optional[ProcessEvents] = Field(
        ...
        description="Flatten and clean event data: timezone conversion, event detail normalisation, column filtering, and optional SQL transformation.",
    )
    ...
    Generate_Maps: Optional[GenerateMaps] = Field(
        None,
        alias="Generate Maps",
        description="Generate maps to visualise events. Skipped by default.",
    )
